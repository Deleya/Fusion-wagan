...
class ChatAPIView(APIView):
    ...
    def post(self,request,format=None):
        ...
        if github:
            # Example Usage
            github_link = "https://github.com/Ahmettidianecisse/Formulaire" #replace with your link.
        ...
        if images:
            noms = []
            for image in images:
                noms.append(image.name)
            #img = PIL.Image.open('design.png')
            return Response({"message": "Data received", "user": name, "response": noms}, status=status.HTTP_200_OK)
